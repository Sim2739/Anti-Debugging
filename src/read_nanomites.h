...
struct packed_file read_in_nanomites()		//creates packed_file struct from nanomites_dump file
{
	...
	//fp = fopen(filename,"r");	
	fp = fmemopen(linux_resc_nanomites_dump,linux_resc_nanomites_dump_len,"r");	//nanomites_dump file is converted into .h file and the unsigned char array is transformed into stream so we can treat it as file
	char line[256];
